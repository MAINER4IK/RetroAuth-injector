...
import moe.yushi.authlibinjector.httpd.LegacyResourcesFilter;
...
import moe.yushi.authlibinjector.httpd.LegacyAuthGameFilter;
import moe.yushi.authlibinjector.httpd.LegacyMapApiFilter;
import moe.yushi.authlibinjector.httpd.LegacyFmlLibsFilter;
...
public final class AuthlibInjector {
	...
	private static List<URLFilter> createFilters(APIMetadata config) {
		...
		filters.add(new LegacyAuthGameFilter());
		filters.add(new LegacyMapApiFilter());
		filters.add(new LegacyFmlLibsFilter());
		filters.add(new LegacyResourcesFilter());
	...
	private static ClassTransformer createTransformer(APIMetadata config) {
		urlProcessor = new URLProcessor(createFilters(config), new DefaultURLRedirector(config));
	...
	public static Optional<String> transformURLForLegacy(String inputUrl) {
		URLProcessor processor = urlProcessor;
		if (processor == null) {
			return Optional.empty();
		}
		return processor.transformURL(inputUrl);
	}
}
